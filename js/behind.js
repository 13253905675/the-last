      ...
      for (var i = 0; i < revise.length; i++) {
        revise[i].onclick = function () {
          ...
          sure.onclick = function () {
            var useNameSure = useName.value;
            useNameSure = judgeStr(useNameSure);
           if(useNameSure==false){
                alert("请重新输入")
           }
            ...
            if(useNameSure!=false &&useAgeS>0&&useAgeS<130)
            {
              $.ajax({
                type: "POST",
    ...
    success: function (result) {
      ...
      for (var k = 0; k < revises.length; k++) {
        revises[k].onclick = function () {
          ...
          var redWord=document.getElementsByClassName('redWord')[0];
          var or=document.getElementsByClassName("or")[0];
          ...
          sureP.onclick = function () {
            var useAccountS=useAccount.value;
            if(useAccountS<0){
            
                redWord.classList.add("redWordP");
                redWord.innerHTML = "请输入正数！";
                return;
              }
              var usePayS=usePay.value;
            if(usePayS!=0&&usePayS!=1){
            
                or.classList.add("orWrong");
                or.innerHTML = "请输入0或1";
                return;
              } 
      ...
      for (var k = 0; k < revisef.length; k++) {
        revisef[k].onclick = function () {
          ...
          sureF.onclick = function () {
            var tasteSure = taste.value;
            tasteSure = judgeStr(tasteSure);
           if(tasteSure==false){
                alert("请重新输入")
                return
           }
          
           var tasteNameSure = typeName.value;
           tasteNameSure = judgeStr(tasteNameSure);
          if(tasteNameSure==false){
               alert("请重新输入")
               return
          }
            if (foodPay.value < 0) {
...
var moneyZ=document.getElementsByClassName("moneyZ")[0];
addFB.onclick = function () {
  ...
  fts.onclick = function () {
    ...
    var foodPayS=foodPayA.value;
    if(foodPayS<0){
      moneyZ.classList.add("moneyWrong");
      moneyZ.innerHTML="请输入正数";
      return;
    }
